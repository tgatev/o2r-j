...
/**
 * Currency_rates Model
 */
class OfrsModelCurrency_rates extends JModelList
{
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('#__ofrs_currency_rate', 'a'));

		// From the ofrs_currency table.
		$query->select($db->quoteName('g.name','currency_id_name'));
		$query->join('LEFT', $db->quoteName('#__ofrs_currency', 'g') . ' ON (' . $db->quoteName('a.currency_id') . ' = ' . $db->quoteName('g.id') . ')');
	...
	public function getExportData($pks, $user = null)
	{
		...
		if (OfrsHelper::checkArray($pks))
		{
			...
			// From the ofrs_currency_rate table
			$query->from($db->quoteName('#__ofrs_currency_rate', 'a'));
	...
	*/
	public function getExImPortHeaders()
	{
		...
		$db = JFactory::getDbo();
		// get the columns
		$columns = $db->getTableColumns("#__ofrs_currency_rate");
	...
	protected function checkInNow()
	{
		...
		if ($time)
		{
			...
			$query = $db->getQuery(true);
			$query->select('*');
			$query->from($db->quoteName('#__ofrs_currency_rate'));
			$db->setQuery($query);
			$db->execute();
			if ($db->getNumRows())
			{
				...
				// Check table
				$query->update($db->quoteName('#__ofrs_currency_rate'))->set($fields)->where($conditions); 
