 ...
 * Currencies Model
 */
class OfrsModelCurrencies extends JModelList
{
	...
	*/
	...
	protected function checkInNow()
	{
		...
		if ($time)
		{
			...
			$query = $db->getQuery(true);
			$query->select('*');
			$query->from($db->quoteName('#__ofrs_currency'));
			$db->setQuery($query);
			$db->execute();
			if ($db->getNumRows())
			{
				...
				// Check table
				$query->update($db->quoteName('#__ofrs_currency'))->set($fields)->where($conditions); 
