 ...
 * Offer_payouts Model
 */
class OfrsModelOffer_payouts extends JModelList
{
	public function __construct($config = array())
	{
		if (empty($config['filter_fields']))
        {
			$config['filter_fields'] = array(
				...
				'a.percent_payout','percent_payout'
			);
		}
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$payout = $this->getUserStateFromRequest($this->context . '.filter.payout', 'filter_payout');
		$this->setState('filter.payout', $payout);
		...
		$percent_payout = $this->getUserStateFromRequest($this->context . '.filter.percent_payout', 'filter_percent_payout');
		$this->setState('filter.percent_payout', $percent_payout);
	...
	public function getItems()
	{
		...
		if (OfrsHelper::checkArray($items))
		{
			...
			{
				// convert payout_type
				$item->payout_type = OfrsHelper::jsonToString($item->payout_type, ', ', 'payout_type', 'id', 'name');
			}
		}
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('#__ofrs_offer_payout', 'a'));
		...
		if ($currency_id = $this->getState('filter.currency_id'))
		{
			$query->where('a.currency_id = ' . $db->quote($db->escape($currency_id)));
		}
	...
	public function getExportData($pks, $user = null)
	{
		...
		if (OfrsHelper::checkArray($pks))
		{
			...
			// From the ofrs_offer_payout table
			$query->from($db->quoteName('#__ofrs_offer_payout', 'a'));
	...
	*/
	public function getExImPortHeaders()
	{
		...
		$db = JFactory::getDbo();
		// get the columns
		$columns = $db->getTableColumns("#__ofrs_offer_payout");
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':' . $this->getState('filter.payout');
		$id .= ':' . $this->getState('filter.currency_id');
		$id .= ':' . $this->getState('filter.percent_payout');
	...
	protected function checkInNow()
	{
		...
		if ($time)
		{
			...
			$query = $db->getQuery(true);
			$query->select('*');
			$query->from($db->quoteName('#__ofrs_offer_payout'));
			$db->setQuery($query);
			$db->execute();
			if ($db->getNumRows())
			{
				...
				// Check table
				$query->update($db->quoteName('#__ofrs_offer_payout'))->set($fields)->where($conditions); 
