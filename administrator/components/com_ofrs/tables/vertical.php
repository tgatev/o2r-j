...
/**
 * Verticals Table class
 */
class OfrsTableVertical extends JTable
{
	/**
	...
	{
		parent::__construct('#__ofrs_vertical', 'id', $db);
	}	
	...
	/**
	 * Overload the store method for the Vertical table.
	 *
	...
	public function store($updateNulls = false)
	{
		...
		{
			// New vertical. A vertical created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			$table = JTable::getInstance('vertical', 'OfrsTable');

			if ($table->load(array('alias' => $this->alias)) && ($table->id != $this->id || $this->id == 0))
			{
				$this->setError(JText::_('COM_OFRS_VERTICAL_ERROR_UNIQUE_ALIAS'));
				return false;
			}
		}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_ofrs.vertical.'.$this->id);
			$this->setRules($rules);
		}
	...
	{
		$k = $this->_tbl_key;
		return 'com_ofrs.vertical.'.(int) $this->$k;
	}
