 ...
 * Offers Table class
 */
class OfrsTableOffer extends JTable
{
	/**
	...
	{
		parent::__construct('#__ofrs_offer', 'id', $db);
	}	
	...
	/**
	 * Overload the store method for the Offer table.
	 *
	...
	public function store($updateNulls = false)
	{
		...
		{
			// New offer. A offer created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			$table = JTable::getInstance('offer', 'OfrsTable');

			if ($table->load(array('alias' => $this->alias)) && ($table->id != $this->id || $this->id == 0))
			{
				$this->setError(JText::_('COM_OFRS_OFFER_ERROR_UNIQUE_ALIAS'));
				return false;
			}
		}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_ofrs.offer.'.$this->id);
			$this->setRules($rules);
		}
	...
	{
		$k = $this->_tbl_key;
		return 'com_ofrs.offer.'.(int) $this->$k;
	}
