...
/**
 * Ad_networks Table class
 */
class OfrsTableAd_network extends JTable
{
	/**
	...
	{
		parent::__construct('#__ofrs_ad_network', 'id', $db);
	}	
	...
	/**
	 * Overload the store method for the Ad_network table.
	 *
	...
	public function store($updateNulls = false)
	{
		...
		{
			// New ad_network. A ad_network created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			$table = JTable::getInstance('ad_network', 'OfrsTable');

			if ($table->load(array('alias' => $this->alias)) && ($table->id != $this->id || $this->id == 0))
			{
				$this->setError(JText::_('COM_OFRS_AD_NETWORK_ERROR_UNIQUE_ALIAS'));
				return false;
			}
		}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_ofrs.ad_network.'.$this->id);
			$this->setRules($rules);
		}
	...
	{
		$k = $this->_tbl_key;
		return 'com_ofrs.ad_network.'.(int) $this->$k;
	}
