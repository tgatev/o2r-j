...
/**
 * Offer_payouts Table class
 */
class OfrsTableOffer_payout extends JTable
{
	/**
	...
	{
		parent::__construct('#__ofrs_offer_payout', 'id', $db);
	}	
	...
	/**
	 * Overload the store method for the Offer_payout table.
	 *
	...
	public function store($updateNulls = false)
	{
		...
		{
			// New offer_payout. A offer_payout created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			$table = JTable::getInstance('offer_payout', 'OfrsTable');

			if ($table->load(array('alias' => $this->alias)) && ($table->id != $this->id || $this->id == 0))
			{
				$this->setError(JText::_('COM_OFRS_OFFER_PAYOUT_ERROR_UNIQUE_ALIAS'));
				return false;
			}
		}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_ofrs.offer_payout.'.$this->id);
			$this->setRules($rules);
		}
	...
	{
		$k = $this->_tbl_key;
		return 'com_ofrs.offer_payout.'.(int) $this->$k;
	}
