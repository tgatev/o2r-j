...
/**
 * Currency_rates Table class
 */
class OfrsTableCurrency_rate extends JTable
{
	/**
	...
	{
		parent::__construct('#__ofrs_currency_rate', 'id', $db);
	}	
	...
	/**
	 * Overload the store method for the Currency_rate table.
	 *
	...
	public function store($updateNulls = false)
	{
		...
		{
			// New currency_rate. A currency_rate created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			$table = JTable::getInstance('currency_rate', 'OfrsTable');

			if ($table->load(array('alias' => $this->alias)) && ($table->id != $this->id || $this->id == 0))
			{
				$this->setError(JText::_('COM_OFRS_CURRENCY_RATE_ERROR_UNIQUE_ALIAS'));
				return false;
			}
		}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_ofrs.currency_rate.'.$this->id);
			$this->setRules($rules);
		}
	...
	{
		$k = $this->_tbl_key;
		return 'com_ofrs.currency_rate.'.(int) $this->$k;
	}
