...
/**
 * Imp_messages Table class
 */
class OfrsTableImp_message extends JTable
{
	/**
	...
	{
		parent::__construct('#__ofrs_imp_message', 'id', $db);
	}	
	...
	/**
	 * Overload the store method for the Imp_message table.
	 *
	...
	public function store($updateNulls = false)
	{
		...
		{
			// New imp_message. A imp_message created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			$table = JTable::getInstance('imp_message', 'OfrsTable');

			if ($table->load(array('alias' => $this->alias)) && ($table->id != $this->id || $this->id == 0))
			{
				$this->setError(JText::_('COM_OFRS_IMP_MESSAGE_ERROR_UNIQUE_ALIAS'));
				return false;
			}
		}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_ofrs.imp_message.'.$this->id);
			$this->setRules($rules);
		}
	...
	{
		$k = $this->_tbl_key;
		return 'com_ofrs.imp_message.'.(int) $this->$k;
	}
