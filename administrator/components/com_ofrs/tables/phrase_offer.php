...
/**
 * Phrase_offers Table class
 */
class OfrsTablePhrase_offer extends JTable
{
	/**
	...
	{
		parent::__construct('#__ofrs_phrase_offer', 'id', $db);
	}	
	...
	/**
	 * Overload the store method for the Phrase_offer table.
	 *
	...
	public function store($updateNulls = false)
	{
		...
		{
			// New phrase_offer. A phrase_offer created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			$table = JTable::getInstance('phrase_offer', 'OfrsTable');

			if ($table->load(array('alias' => $this->alias)) && ($table->id != $this->id || $this->id == 0))
			{
				$this->setError(JText::_('COM_OFRS_PHRASE_OFFER_ERROR_UNIQUE_ALIAS'));
				return false;
			}
		}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_ofrs.phrase_offer.'.$this->id);
			$this->setRules($rules);
		}
	...
	{
		$k = $this->_tbl_key;
		return 'com_ofrs.phrase_offer.'.(int) $this->$k;
	}
