 ...
 * Phrases Table class
 */
class OfrsTablePhrase extends JTable
{
	/**
	...
	{
		parent::__construct('#__ofrs_phrase', 'id', $db);
	}	
	...
	/**
	 * Overload the store method for the Phrase table.
	 *
	...
	public function store($updateNulls = false)
	{
		...
		{
			// New phrase. A phrase created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			$table = JTable::getInstance('phrase', 'OfrsTable');

			if ($table->load(array('alias' => $this->alias)) && ($table->id != $this->id || $this->id == 0))
			{
				$this->setError(JText::_('COM_OFRS_PHRASE_ERROR_UNIQUE_ALIAS'));
				return false;
			}
		}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_ofrs.phrase.'.$this->id);
			$this->setRules($rules);
		}
	...
	{
		$k = $this->_tbl_key;
		return 'com_ofrs.phrase.'.(int) $this->$k;
	}
