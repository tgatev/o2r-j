...
/**
 * Tracking_platforms Table class
 */
class OfrsTableTracking_platform extends JTable
{
	/**
	...
	{
		parent::__construct('#__ofrs_tracking_platform', 'id', $db);
	}	
	...
	/**
	 * Overload the store method for the Tracking_platform table.
	 *
	...
	public function store($updateNulls = false)
	{
		...
		{
			// New tracking_platform. A tracking_platform created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			$table = JTable::getInstance('tracking_platform', 'OfrsTable');

			if ($table->load(array('alias' => $this->alias)) && ($table->id != $this->id || $this->id == 0))
			{
				$this->setError(JText::_('COM_OFRS_TRACKING_PLATFORM_ERROR_UNIQUE_ALIAS'));
				return false;
			}
		}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_ofrs.tracking_platform.'.$this->id);
			$this->setRules($rules);
		}
	...
	{
		$k = $this->_tbl_key;
		return 'com_ofrs.tracking_platform.'.(int) $this->$k;
	}
