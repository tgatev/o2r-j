...
/**
 * Imp_sessions Table class
 */
class OfrsTableImp_session extends JTable
{
	/**
	...
	{
		parent::__construct('#__ofrs_imp_session', 'id', $db);
	}	
	...
	/**
	 * Overload the store method for the Imp_session table.
	 *
	...
	public function store($updateNulls = false)
	{
		...
		{
			// New imp_session. A imp_session created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			$table = JTable::getInstance('imp_session', 'OfrsTable');

			if ($table->load(array('alias' => $this->alias)) && ($table->id != $this->id || $this->id == 0))
			{
				$this->setError(JText::_('COM_OFRS_IMP_SESSION_ERROR_UNIQUE_ALIAS'));
				return false;
			}
		}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_ofrs.imp_session.'.$this->id);
			$this->setRules($rules);
		}
	...
	{
		$k = $this->_tbl_key;
		return 'com_ofrs.imp_session.'.(int) $this->$k;
	}
