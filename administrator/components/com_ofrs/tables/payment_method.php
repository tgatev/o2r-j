...
/**
 * Payment_methods Table class
 */
class OfrsTablePayment_method extends JTable
{
	/**
	...
	{
		parent::__construct('#__ofrs_payment_method', 'id', $db);
	}	
	...
	/**
	 * Overload the store method for the Payment_method table.
	 *
	...
	public function store($updateNulls = false)
	{
		...
		{
			// New payment_method. A payment_method created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			$table = JTable::getInstance('payment_method', 'OfrsTable');

			if ($table->load(array('alias' => $this->alias)) && ($table->id != $this->id || $this->id == 0))
			{
				$this->setError(JText::_('COM_OFRS_PAYMENT_METHOD_ERROR_UNIQUE_ALIAS'));
				return false;
			}
		}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_ofrs.payment_method.'.$this->id);
			$this->setRules($rules);
		}
	...
	{
		$k = $this->_tbl_key;
		return 'com_ofrs.payment_method.'.(int) $this->$k;
	}
