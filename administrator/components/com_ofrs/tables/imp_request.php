...
/**
 * Imp_requests Table class
 */
class OfrsTableImp_request extends JTable
{
	/**
	...
	{
		parent::__construct('#__ofrs_imp_request', 'id', $db);
	}	
	...
	/**
	 * Overload the store method for the Imp_request table.
	 *
	...
	public function store($updateNulls = false)
	{
		...
		{
			// New imp_request. A imp_request created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			$table = JTable::getInstance('imp_request', 'OfrsTable');

			if ($table->load(array('alias' => $this->alias)) && ($table->id != $this->id || $this->id == 0))
			{
				$this->setError(JText::_('COM_OFRS_IMP_REQUEST_ERROR_UNIQUE_ALIAS'));
				return false;
			}
		}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_ofrs.imp_request.'.$this->id);
			$this->setRules($rules);
		}
	...
	{
		$k = $this->_tbl_key;
		return 'com_ofrs.imp_request.'.(int) $this->$k;
	}
