...
/**
 * Phrase_verticals Table class
 */
class OfrsTablePhrase_vertical extends JTable
{
	/**
	...
	{
		parent::__construct('#__ofrs_phrase_vertical', 'id', $db);
	}	
	...
	/**
	 * Overload the store method for the Phrase_vertical table.
	 *
	...
	public function store($updateNulls = false)
	{
		...
		{
			// New phrase_vertical. A phrase_vertical created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			$table = JTable::getInstance('phrase_vertical', 'OfrsTable');

			if ($table->load(array('alias' => $this->alias)) && ($table->id != $this->id || $this->id == 0))
			{
				$this->setError(JText::_('COM_OFRS_PHRASE_VERTICAL_ERROR_UNIQUE_ALIAS'));
				return false;
			}
		}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_ofrs.phrase_vertical.'.$this->id);
			$this->setRules($rules);
		}
	...
	{
		$k = $this->_tbl_key;
		return 'com_ofrs.phrase_vertical.'.(int) $this->$k;
	}
