...
/**
 * Imp_sessions Controller
 */
class OfrsControllerImp_sessions extends JControllerAdmin
{
	/**
	 ...
	 * @since  1.6
	 */
	protected $text_prefix = 'COM_OFRS_IMP_SESSIONS';
	...
	public function exportData()
	{
		...
		if ($user->authorise('imp_session.export', 'com_ofrs') && $user->authorise('core.export', 'com_ofrs'))
		{
			...
			// Get the model
			$model = $this->getModel('Imp_sessions');
			...
			if (OfrsHelper::checkArray($data))
			{
				...
				OfrsHelper::xls($data,'Imp_sessions_'.$date->format('jS_F_Y'),'Imp sessions exported ('.$date->format('jS F, Y').')','imp sessions');
			}
		}
		// Redirect to the list screen with error.
		$message = JText::_('COM_OFRS_EXPORT_FAILED');
		$this->setRedirect(JRoute::_('index.php?option=com_ofrs&view=imp_sessions', false), $message, 'error');
		return;
	}
	...
	public function importData()
	{
		...
		if ($user->authorise('imp_session.import', 'com_ofrs') && $user->authorise('core.import', 'com_ofrs'))
		{
			...
			if (OfrsHelper::checkObject($headers))
			{
				...
				$session->set('backto_VDM_IMPORT', 'imp_sessions');
				$session->set('dataType_VDM_IMPORTINTO', 'imp_session');
				// Redirect to import view.
				$message = JText::_('COM_OFRS_IMPORT_SELECT_FILE_FOR_IMP_SESSIONS');
