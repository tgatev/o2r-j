...
class UuModelSubscriptions extends JModelList {
    ...
    public function getItems($sql) {
        ...
        $sql = str_replace('@u', $this->userId, $sql);
        $query -> select($sql);
    ...
    public function getAdnetsMonitored() {
        $items = $this->getItems("n.id AS adnet_id, n.name AS adnet_name, n.name_text_color as name_text_color, n.name_background_color as name_background_color
                        FROM jc_ofrs_ad_network as n join ofrs_ad_network_monitor as nm on (n.id = nm.ad_network_id) 
                        WHERE nm.user_id = @u
                       ");
        return  $items;
    }
