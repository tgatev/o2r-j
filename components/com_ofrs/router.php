...
/**
 * Routing class from com_ofrs
 *
 * @since  3.3
 */
class OfrsRouter extends JComponentRouterBase
{	
	/**
	 * Build the route for the com_ofrs component
	 *
	...
	public function build(&$query)
	{
		...
		if (isset($view) && isset($query['id']) && ($view === 'adnets' || $view === 'offers' || $view === 'adnet' || $view === 'offer'))
		{
			if ($mId != (int) $query['id'] || $mView != $view)
			{
				if (($view === 'adnets' || $view === 'offers' || $view === 'adnet' || $view === 'offer'))
				{
					$segments[] = $view;
					$id = explode(':', $query['id']);
					if (count($id) == 2)
	...
	{		
		...
		switch($segments[0])
		{
			case 'adnets':
				...
				{
					$id = $this->getVar('adnets', $segments[$count-1], 'alias', 'id');
					if($id)
					{
						$vars['id'] = $id;
					}
			...
			case 'offers':
				...
				elseif ($segments[$count-1])
				{
					$id = $this->getVar('offers', $segments[$count-1], 'alias', 'id');
					if($id)
					{
						$vars['id'] = $id;
					}
			...
			case 'adnet':
				...
				elseif ($segments[$count-1])
				{
					$id = $this->getVar('ad_network', $segments[$count-1], 'alias', 'id');
					if($id)
					{
						$vars['id'] = $id;
					}
			...
			case 'offer':
				...
				elseif ($segments[$count-1])
				{
					$id = $this->getVar('offer', $segments[$count-1], 'alias', 'id');
					if($id)
					{
						$vars['id'] = $id;
					}
	...
	protected function getVar($table, $where = null, $whereString = null, $what = null, $category = false, $operator = '=', $main = 'ofrs')
	{
...
function OfrsBuildRoute(&$query)
{
	$router = new OfrsRouter;
	
	return $router->build($query);
}

function OfrsParseRoute($segments)
{
	$router = new OfrsRouter;
