...
class OfrsModelAdnets extends JModelList
{
    ...
    protected function populateState($ordering = null, $direction = null)
    {
        ...
        $filter = $app->getUserStateFromRequest('adnets.filter', 'adnet_filter', array());
        $search = $app->getUserStateFromRequest('ofrs.filter', 'filter', ["adnet_search" => ""]);
        $offset = $app->getUserStateFromRequest('list.start', 'limitstart', 0);
        $filter["adnet_search"] = $search["adnet_search"];
